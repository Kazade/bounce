...
void b3StretchForceDef::Initialize(const b3Vec3& p1, const b3Vec3& p2, const b3Vec3& p3)
{
...
b3StretchForce::b3StretchForce(const b3StretchForceDef* def)
{
	...
	m_p2 = def->p2;
	m_p3 = def->p3;
	m_ks_u = def->stretching_u;
	m_kd_u = def->damping_u;
	m_b_u = def->b_u;
	m_ks_v = def->stretching_v;
	m_kd_v = def->damping_v;
	m_b_v = def->b_v;
	m_f1.SetZero();
	m_f2.SetZero();
	m_f3.SetZero();
...
b3StretchForce::~b3StretchForce()
{

}

bool b3StretchForce::HasParticle(const b3ClothParticle* particle) const
{
	return m_p1 == particle || m_p2 == particle || m_p3 == particle;
}

void b3StretchForce::Apply(const b3ClothForceSolverData* data)
{
			...
			if (len_wv > m_b_v)
			{
				...
				for (u32 i = 0; i < 3; ++i)
				{
					for (u32 j = 0; j < 3; ++j)
					{
						...
						b3Mat33 Kij = -m_ks_v * (b3Outer(dCvdx[i], dCvdx[j]) + Cv * d2Cvxij);
